...
app.get('/api/audiobooks/popular', async (req, res) => {
  ...
  try {
    ...
    if (category === 'graphicaudio') {
    ...
    } else if (category === 'abb' || category === 'debrid') {
      const abbHits = await searchAudiobookBay('bestseller');
      results = abbHits.map(b => ({
        id: b.id,
        title: b.title,
        author: 'AudiobookBay Release',
        narrator: 'High-Speed Lossless Stream',
        duration: 'Full Audio',
      ...
      const localBooks = scanLocalAudiobooksFolder();
      results = localBooks;
...
const APK_DOWNLOAD_URL = 'https://github.com/hffgzktbcc-blip/omnistream/releases/download/v1.0.1/OmniStream-AndroidTV.apk';
app.get('/apk', (req, res) => {
